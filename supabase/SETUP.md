...
如果项目已经执行过旧版 `schema.sql`，请在 SQL Editor 中再执行一次
[`migration-v2-site-settings.sql`](./migration-v2-site-settings.sql)。这个脚本会：

- 创建公开可读的 `site_settings` 表；
- 只允许 `site_admins` 中的站主新增、修改或删除设置；
- 让站主可以在网站管理后台保存网易云歌单 ID。

脚本可以重复执行，不会清空已有碎碎念、照片、歌曲或留言。

网易云使用外链播放器。会员权益不能共享给网站访客，会员歌曲能否完整播放由网易云根据访问者登录状态和版权规则决定。
